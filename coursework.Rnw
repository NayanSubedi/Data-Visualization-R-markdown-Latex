...
<<fig=FALSE, echo=FALSE, warning=FALSE, message=FALSE>>=
library(ggplot2)
library(dplyr)
library(tidyverse)
library(reshape2)
...
\begin{figure}[h!]
\centering
<<fig=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=12, fig.height=12>>=
unemployment_data <- world_bank_data %>%
  select(Year, Region, Income_Group, Unemployment_Rate) %>%
  na.omit() %>%
  group_by(Year, Region, Income_Group) %>%
...
\begin{figure}[h!]
\centering
<<fig=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8, fig.height=8>>=
average_electric_power<- world_bank_data %>%
  group_by(Country) %>%
  summarise(Electric_Power_Consumption_per_Capita = mean(Electric_Power_Consumption_per_Capita, na.rm = TRUE))
...
UR_map <- mapCountryData(UR, nameColumnToPlot = "Unemployment_Rate", mapTitle = "Average Unemployment Rate of Labour Force", 
                         colourPalette = my_palette3, missingCountryCol = "black")
